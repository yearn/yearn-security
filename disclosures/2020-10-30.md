...
- A potential vulnerability was mitigated ~1.5 hours after being reported through Yearn's security vulnerability disclosure process on October 29 2020.
...
A security researcher contacts Yearn's security team following the steps outlined in Yearn's [security process document](https://github.com/iearn-finance/yearn-protocol/blob/develop/SECURITY.md#receiving-disclosures).

Having established contact, the researcher discloses an initial proof of concept of a flash loan attack that can be mounted on the TUSD vault, resulting in an 18% loss to users, with the attacker being able to walk away with 650k TUSD.

As a mitigation against the attack, the researcher recommends that the TUSD vault immediately is prevented from sending funds to the affected strategy, by calling `setMin(0)` on the TUSD vault.

The report also notes that other vaults using the Curve strategy would potentially be vulnerable to the same attack, such as the DAI vault and the GUSD vault, but that these vaults already have min set to zero and are therefore not vulnerable.
...
**Oct 30, 2020, 15:56 (UTC):** Initial contact attempt is made with Yearn's security team in line with the established policy.
...
- `ychad.eth` [disabling deposits](https://etherscan.io/tx/0x508b3f5607cbfe25dcc12980c9783920236c087f1ab7591b60c55614d503b546) using `setMin(0)` on the TUSD Vault
- Exploit proof of concept [shared with Yearn](https://gist.github.com/xu3kev/fbdf166ba0e5f9e13522af8d043f37a1).
- Additional proof of concept [which doesn't use a flash loan](https://gist.github.com/banteg/b26ca556308b58d2643299dd02dbb54c).
