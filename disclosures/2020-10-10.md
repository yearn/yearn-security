...
At 11:00 (UTC) a concern is shared that `StrategyProxy.deposit(address gauge, address token)` can be executed by anyone. By constructing a fake gauge contract, it's possible to force `CurveYCRVVoter` to call `deposit(uint amount)` on it with a token allowance.

The concern is initially dismissed as funds should never end up in the proxy. Follow-up review finds that there is in fact 34,352 CRV sitting in the Voter Proxy.

A few moments later, a proof of concept exploit contract confirms the vulnerability in forked mainnet.

The team decides to whitehack the contract to not put funds at risk. The contract is exploited and the recovered funds are sent to multisig.
...
- **Oct 10, 2020, 11:00 UTC** The concern is raised in YFI Secret Admirers chat.
- **12:53** Additional details are shared and an exploit is written.
...
- Whitehack [transaction](https://etherscan.io/tx/0x8302d3481d2b4eb6b5ba7d6b467e3ce8cd3450dac3c8857aac7964bd54ed507a)
